...
// ── Validate stock for a list of items (shared by initiate & COD) ─────────────
async function validateStock(env, items) {
  for (const item of items) {
    if (!item.productId) continue;
    const prod = await env.DB.prepare('SELECT id, name, stock FROM products WHERE id=?').bind(item.productId).first();
    if (!prod) return `Product "${item.name || item.productId}" not found.`;
    ...
    if (sizeLabel) {
      ...
      if (sizeRow) {
        if ((sizeRow.stock || 0) < qty)
          return `Size ${sizeLabel} of "${prod.name}" is out of stock. Only ${sizeRow.stock} left.`;
      } else {
        if ((prod.stock || 0) < qty)
          return `Insufficient stock for "${prod.name}" (size ${sizeLabel}). Only ${prod.stock} left.`;
      }
    } else {
      if ((prod.stock || 0) < qty)
        return `Insufficient stock for "${prod.name}". Only ${prod.stock} left.`;
    }
  }
...
function validateCustomer(customer, deliveryMethod) {
  if (!customer) return 'Customer details are required';
  const isPos = deliveryMethod === 'pos' || deliveryMethod === 'store';
  const name = String(customer.name || '').trim();
  const email = String(customer.email || '').trim();
  const phone = String(customer.phone || '').trim();
  const addr = String(customer.addressLine1 || customer.address_line1 || '').trim();
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return 'Valid customer email is required';
  if (!phone) return 'Customer phone is required';
  if (!isPos && (!addr || !city || !state || !pincode))
    return 'Address, city, state, and pincode are required for delivery orders';
  return null; // valid
}

// ── CREATE ORDER RECORD (Used on successful Razorpay verification & COD) ──────
export async function createOrderRecord(env, input) {
  ...
  const result = await env.DB.prepare(
    `INSERT INTO orders (order_number, user_id, customer_name, customer_email, customer_phone,
     ...
     payment_method, payment_status, order_status, subtotal_amount, shipping_amount, tax_amount,
     discount_amount, total_amount, notes, source, created_at, updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).bind(
...
export async function ordersRouter(request, env) {
  ...
  if (path === '/initiate' && method === 'POST') {
    try {
      ...
      // Validate customer details
      const customerErr = validateCustomer(body.customer, body.deliveryMethod);
      if (customerErr) return error(customerErr, 400);

      // Validate stock
      const stockErr = await validateStock(env, items);
      if (stockErr) return error(stockErr, 400);

      let discountAmount = 0;
      let couponCode = String(body.couponCode || '').trim().toUpperCase();
      const subtotalAmount = Number(
        items.reduce((s, i) => s + (Number(i.price || 0) * Math.max(1, i.qty || i.quantity || 1)), 0).toFixed(2)
      );
      ...
      const resolvedCustomer = body.customer || defaultCustomer;
      ...
      if (amountPaise <= 0) {
        ...
        return ok({
          key: 'free_order',
          order: { id: createdRes.order.id, orderNumber: createdRes.order.order_number, amount: 0, discount: discountAmount },
        });
      }
      ...
      return ok({
        key: rzpKeyId,
        order: { id: null, orderNumber, amount: totalAmount, discount: discountAmount },
  ...
  // ── POST /api/orders/cod — COD order placement ────────────────────────────
  if (path === '/cod' && method === 'POST') {
    try {
      const { user, error: authErr } = await requireAuth(request, env);
      if (authErr) return authErr;
      ...
      if (couponCode) {
        const coupon = await env.DB.prepare('SELECT * FROM coupons WHERE code=? AND active = 1').bind(couponCode).first();
        if (!coupon) return error('Invalid or inactive coupon code', 400);
        if (subtotalAmount < (coupon.min_order || 0))
          return error(`Minimum order of ₹${coupon.min_order} required for this coupon`, 400);
        let disc = coupon.type === 'percent'
          ? Math.round(subtotalAmount * (coupon.value / 100))
          : coupon.value;
        if (coupon.max_discount) disc = Math.min(disc, coupon.max_discount);
      ...
      // Fetch the full created order to return
      const fullOrder = await env.DB.prepare('SELECT * FROM orders WHERE id=?').bind(orderId).first();
      const itemsRes = await env.DB.prepare('SELECT * FROM order_items WHERE order_id=?').bind(orderId).all();

      return created(
        formatOrder(fullOrder, (itemsRes.results || []).map(formatItem)),
        'COD order placed successfully'
      );
    } catch (e) {
      console.error('COD order error:', e);
      return serverError('Failed to place COD order');
    }
  }
  ...
  if (path === '/my' && method === 'GET') {
    ...
    try {
      ...
      // Fix N+1: fetch all items for all orders in one query
      let itemsByOrderId = {};
      if (orderRows.length > 0) {
        const orderIds = orderRows.map(o => o.id);
        const placeholders = orderIds.map(() => '?').join(',');
        const allItemsRes = await env.DB.prepare(
          `SELECT order_id, product_name, image_url, size_label, quantity, unit_price
           FROM order_items WHERE order_id IN (${placeholders}) ORDER BY order_id, id ASC`
        ).bind(...orderIds).all();

        for (const it of (allItemsRes.results || [])) {
          if (!itemsByOrderId[it.order_id]) itemsByOrderId[it.order_id] = [];
          // Limit to 4 preview items per order
          if (itemsByOrderId[it.order_id].length < 4) {
      ...
      const orders = orderRows.map(o => ({ ...formatOrder(o), items: itemsByOrderId[o.id] || [] }));
  ...
  if (path.startsWith('/track/') && method === 'GET') {
    ...
    try {
      const order = await env.DB.prepare(
        `SELECT order_number, order_status, payment_status, tracking_number, tracking_url,
  ...
  // ── GET /api/orders/:id/invoice — structured invoice data ────────────────
  if (path.match(/^\/\d+\/invoice$/) && method === 'GET') {
    const { user, error: authErr } = await requireAuth(request, env);
    if (authErr) return authErr;
    const id = parseInt(path.match(/(\d+)/)?.[1]);
    try {
      // Customers can only view their own invoice; admins can view all
      const isAdmin = ['admin', 'staff', 'manager'].includes(user.role);
      const order = isAdmin
        ? await env.DB.prepare('SELECT * FROM orders WHERE id = ?').bind(id).first()
        : await env.DB.prepare('SELECT * FROM orders WHERE id = ? AND user_id = ?').bind(id, user.id).first();
      if (!order) return notFound('Order not found');
  ...
  if (path.match(/^\/\d+\/return$/) && method === 'POST') {
    const { user, error: authErr } = await requireAuth(request, env);
    if (authErr) return authErr;
    const id = parseInt(path.match(/(\d+)/)?.[1]);
    try {
      ...
      if (order.order_status !== 'delivered')
        return error('Return can only be requested for delivered orders', 400);
      ...
      if (order.delivered_at) {
        ...
        if (daysPassed > windowDays)
          return error(`Return window of ${windowDays} days has expired`, 400);
      }
      ...
      if (existing) return error('A return request already exists for this order', 409);

      const body = await request.json();
      const { reason, description = '', items = [] } = body;
      if (!reason?.trim()) return error('Return reason is required', 400);

      const result = await env.DB.prepare(
        `INSERT INTO returns (order_id, user_id, reason, description, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, 'pending', datetime('now'), datetime('now'))
         RETURNING *`
      ).bind(id, user.id, reason.trim(), description).first();

      // Save return items if provided
      for (const item of items) {
        await env.DB.prepare(
      ...
      return created(result, 'Return request submitted successfully');
    } catch (e) {
  ...
  // ── PUT/PATCH /api/orders/admin/:id/status ────────────────────────────────
  if (path.match(/^\/admin\/\d+\/status$/) && (method === 'PUT' || method === 'PATCH')) {
    ...
    try {
      ...
      if (!currentOrder) return notFound('Order not found');
      ...
      if (tracking_number !== undefined) { sets.push('tracking_number = ?'); binds.push(tracking_number); }
      if (tracking_url !== undefined) { sets.push('tracking_url = ?'); binds.push(tracking_url); }
