...
// ── Per-route Cache-Control TTLs (seconds) ───────────────────────────────────
function getCacheTtl(pathNormalized) {
  if (pathNormalized === '/api/settings')                                  return 3600; // 1 hour
  if (pathNormalized === '/api/categories')                                return 600;  // 10 min
  if (pathNormalized === '/api/brands')                                    return 600;  // 10 min
  if (pathNormalized === '/api/banners')                                   return 300;  // 5 min
  if (pathNormalized === '/api/products')                                  return 180;  // 3 min
  if (/^\/api\/products\/\d+$/.test(pathNormalized))                      return 120;  // 2 min
  if (/^\/api\/products\/\d+\/reviews$/.test(pathNormalized))             return 120;  // 2 min
  return 60; // default 60s for any other cacheable path
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 2. Sitemap — served BEFORE the /api check so it gets the right Content-Type
    if (path === '/sitemap.xml') {
    ...
    if (path === '/api/health') {
      return addCors(
        json({ success: true, message: 'HeelsUp API is running!', ts: new Date().toISOString() }),
        request
      );
    }
    ...
    if (path.startsWith('/api/')) {
      ...
      // Determine if this GET request is eligible for Cloudflare Cache API
      const isCacheable =
        method === 'GET' &&
        !pathNormalized.startsWith('/api/admin') &&
        (
          pathNormalized === '/api/banners'    ||
          pathNormalized === '/api/categories' ||
          pathNormalized === '/api/settings'   ||
          pathNormalized === '/api/products'   ||
          pathNormalized === '/api/brands'     ||
      ...
      if (rlRes) return addCors(rlRes, request);
      ...
      try {
        ...
        else if (path.startsWith('/api/brands'))       response = await brandsRouter(request, env);
      ...
      const securedResponse = new Response(corsResponse.body, corsResponse);
      for (const [k, v] of Object.entries(SECURITY_HEADERS)) {
        securedResponse.headers.set(k, v);
      }

      // Store in Cloudflare Cache if eligible
      if (isCacheable && response && response.status === 200 && cache && cacheKey) {
        try {
          const ttl          = getCacheTtl(pathNormalized);
          const cacheableRes = new Response(securedResponse.clone().body, securedResponse);
          cacheableRes.headers.set('Cache-Control', `public, max-age=${ttl}`);
    ...
    // 5. Static files via Cloudflare Assets
    ...
    if (url.pathname.endsWith('.html') || !url.pathname.includes('.')) {
      headers.set(
        ...
        "default-src 'self' https://*.razorpay.com https://fonts.googleapis.com https://fonts.gstatic.com https://cdnjs.cloudflare.com; " +
        "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdnjs.cloudflare.com; " +
        "img-src 'self' data: https://media.heelsup.in https://*.unsplash.com https://*.razorpay.com; " +
        "script-src 'self' 'unsafe-inline' https://checkout.razorpay.com; " +
        "frame-src https://*.razorpay.com; " +
        "connect-src 'self' https://heelsupnew.heelsup.workers.dev https://api.razorpay.com;"
      );
    }
